...
import { View, Text, StyleSheet, Pressable, Switch, ScrollView, Alert, Platform, ActivityIndicator } from 'react-native';
...
const TYPE_OPTIONS: { key: FeatureType; label: string; icon: keyof typeof Ionicons.glyphMap }[] = [
...
function sanitizeFileName(name: string) {
  return name.replace(/[^a-zA-Z0-9\u0600-\u06FF_-]+/g, '_').slice(0, 60) || 'project';
}
...
export default function ExportScreen() {
  ...
  const [includeTypes, setIncludeTypes] = useState<Record<FeatureType, boolean>>({
    point: true,
    building: true,
    line: true,
    polygon: true,
  ...
  const [lastResult, setLastResult] = useState<{ zoneNumber: number; hemisphere: string; count: number } | null>(null);
  ...
  const exportDXF = async () => {
    ...
    if (filteredFeatures.length === 0) {
      Alert.alert('لا توجد عناصر', 'اختر نوعاً واحداً على الأقل يحتوي على عناصر للتصدير.');
      return;
    }
    try {
      setExporting(true);
      ...
      setLastResult({ zoneNumber: result.zoneNumber, hemisphere: result.hemisphere, count: result.featureCount });
    } catch (e: any) {
  ...
  const exportGeoJSON = async () => {
    ...
    const geojson = {
      ...
      features: filteredFeatures.map((f) => ({
        type: 'Feature',
        properties: { name: f.name, category: f.category, source: f.source, notes: f.notes ?? '' },
        geometry:
          f.type === 'point'
    ...
    try {
      if (Platform.OS === 'web') {
        ...
        if (await Sharing.isAvailableAsync()) {
          await Sharing.shareAsync(fileUri, { mimeType: 'application/geo+json', dialogTitle: 'مشاركة GeoJSON' });
        }
      }
    } catch (e: any) {
    ...
    <SafeAreaView style={[styles.flex, { backgroundColor: palette.bg }]} edges={['top']}>
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <Text style={[styles.headerTitle, { color: palette.text }]}>تصدير هندسي DXF</Text>
          <Text style={{ color: palette.textMuted, fontSize: 12 }}>
            ملف DXF حقيقي (AutoCAD R12) بإحداثيات UTM مترية دقيقة - يفتح مباشرة في AutoCAD وCivil 3D وQGIS
          </Text>
        </View>

        {features.length === 0 ? (
          <EmptyState icon="document-outline" title="لا توجد بيانات للتصدير" subtitle="أضف عناصر من شاشة الخريطة أولاً" />
        ) : (
          <>
            <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}>
              <Text style={[styles.cardTitle, { color: palette.text }]}>اختر طبقات التصدير</Text>
              {TYPE_OPTIONS.map((opt) => (
                <View key={opt.key} style={styles.rowBetween}>
                  <Switch
                    value={includeTypes[opt.key]}
                    onValueChange={(v) => setIncludeTypes((s) => ({ ...s, [opt.key]: v }))}
                    trackColor={{ true: palette.primary }}
                  />
                  <View style={styles.rowLabel}>
                    <Ionicons name={opt.icon} size={18} color={palette.primary} />
                    <Text style={{ color: palette.text, fontSize: 13, fontWeight: '600', marginRight: 8 }}>
                      {opt.label}
                    </Text>
            ...
            <View style={[styles.card, { backgroundColor: palette.card, borderColor: palette.border }]}>
              <Text style={[styles.cardTitle, { color: palette.text }]}>مصدر البيانات</Text>
              <View style={styles.rowBetween}>
                <Switch value={includeManual} onValueChange={setIncludeManual} trackColor={{ true: palette.primary }} />
                <Text style={{ color: palette.text, fontSize: 13, fontWeight: '600' }}>عناصر يدوية</Text>
              </View>
              <View style={styles.rowBetween}>
                <Switch value={includeOSM} onValueChange={setIncludeOSM} trackColor={{ true: palette.primary }} />
                <Text style={{ color: palette.text, fontSize: 13, fontWeight: '600' }}>عناصر من OpenStreetMap</Text>
              </View>
            </View>

            <View style={[styles.summaryCard, { backgroundColor: palette.primary + '15', borderColor: palette.primary + '40' }]}>
              <Ionicons name="information-circle" size={20} color={palette.primary} />
              <Text style={{ color: palette.text, fontSize: 12, flex: 1, textAlign: 'right', marginRight: 8, lineHeight: 18 }}>
                سيتم تصدير {filteredFeatures.length} عنصر بنظام إحداثيات UTM (WGS84) المتري، مع طبقات منفصلة لكل نوع وألوان
                مطابقة لمعايير الرسم الهندسي (CAD).
              </Text>
            </View>
            ...
            <Pressable
              onPress={exportDXF}
              disabled={exporting}
              style={[styles.exportBtn, { backgroundColor: palette.primary, opacity: exporting ? 0.7 : 1 }]}
            >
              {exporting ? <ActivityIndicator color="#fff" /> : <Ionicons name="download" size={20} color="#fff" />}
              <Text style={styles.exportBtnText}>{exporting ? 'جارٍ إنشاء الملف...' : 'تصدير ملف DXF'}</Text>
            </Pressable>
            ...
            {lastResult && (
              <View style={[styles.resultCard, { backgroundColor: palette.card, borderColor: palette.border }]}>
                ...
                <Text style={{ color: palette.text, fontSize: 12, textAlign: 'right', flex: 1, marginRight: 8 }}>
            ...
            <View style={[styles.specsCard, { borderColor: palette.border }]}>
              <Text style={[styles.specsTitle, { color: palette.textMuted }]}>مواصفات الملف الفنية</Text>
              {[
                ...
                <View key={i} style={styles.specRow}>
                  <Ionicons name="ellipse" size={5} color={palette.textMuted} style={{ marginTop: 6 }} />
                  <Text style={{ color: palette.textMuted, fontSize: 11, flex: 1, textAlign: 'right', marginRight: 6, lineHeight: 17 }}>
                    {line}
                  </Text>
                </View>
              ))}
...
const styles = StyleSheet.create({
  flex: { flex: 1 },
  header: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 16, alignItems: 'flex-end' },
  headerTitle: { fontSize: 22, fontWeight: '800', marginBottom: 4 },
  card: { marginHorizontal: 20, borderRadius: 16, borderWidth: 1, padding: 16, marginBottom: 14 },
  cardTitle: { fontSize: 14, fontWeight: '800', textAlign: 'right', marginBottom: 12 },
  rowBetween: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 },
  rowLabel: { flexDirection: 'row-reverse', alignItems: 'center', flex: 1, marginRight: 10 },
  summaryCard: {
  ...
  specsCard: { marginHorizontal: 20, borderRadius: 14, borderWidth: 1, padding: 14 },
  specsTitle: { fontSize: 11, fontWeight: '800', textAlign: 'right', marginBottom: 8 },
